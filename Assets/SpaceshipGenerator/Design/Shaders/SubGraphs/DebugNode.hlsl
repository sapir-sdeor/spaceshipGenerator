...
float PrintValue(float2 vStringCoords, float fValue, float fMaxDigits, float fDecimalPlaces)
{
    ...
    if (fDigitIndex > (-fDecimalPlaces - 1.01))
    {
        ...
        else
        {
            ...
            {
                int fReducedRangeValue = int(floor(fValue * 1000.0));
                if(fDigitIndex < 0.0) { fReducedRangeValue = fmod(fReducedRangeValue, 1000.0); fDigitIndex += 1.0; }
                float fDigitValue = fReducedRangeValue / pow(10.0, 3 + fDigitIndex);
                fCharBin = DigitBin(int(floor(fmod(fDigitValue, 10.0) + 0.0002)));
            }
        }
    }
...
{
    res = PrintValue(uv*200, float2(5,95), float2(8,16), val, 10, 3);
}
